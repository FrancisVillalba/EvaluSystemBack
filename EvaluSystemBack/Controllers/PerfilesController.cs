...
public class PerfilesController : ControllerBase
{
    ...
    public PerfilesController(EvaluSystemDbContext context)
    {
        _context = context;
    }
    ...
    {
        var items = await _context.Perfiles.AsNoTracking().ToListAsync();
    ...
    {
        var item = await _context.Perfiles.AsNoTracking().FirstOrDefaultAsync(x => x.Id == id);
        return item is null ? NotFound() : Ok(item.ToDto());
    }

    [HttpPost]
    public async Task<ActionResult<PerfilDto>> Create(PerfilRequest request)
    {
        var item = request.ToEntity();
        _context.Perfiles.Add(item);
        await _context.SaveChangesAsync();
        return CreatedAtAction(nameof(GetById), new { id = item.Id }, item.ToDto());
    }

    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, PerfilRequest request)
    {
        var item = await _context.Perfiles.FindAsync(id);
        if (item is null)
        {
            return NotFound();
        }
